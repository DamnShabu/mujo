...
// Virtual Machines & Visual Server Panel (WP-VM).
// Hypervisor management, ready-to-deploy OS presets (Windows, Ubuntu, Fedora, Arch, macOS),
// custom ISO installer, and zero-latency SPICE visual server integration.
Item {
    id: root
    ...
    Flickable {
        ...
        ColumnLayout {
            ...
            MujoHero {
                ...
                subtitle: "Zero-config OS sandboxes, native SPICE visual server, KVM hardware acceleration, and custom ISO provisioning."
                badgeText: root.vmData.kvm ? "KVM ACCELERATED" : "SOFTWARE EMULATION"
                badgeColor: root.vmData.kvm ? Theme.success : Theme.warning
                activeState: root.runningOp || (root.vmData.activeCount && root.vmData.activeCount > 0)
            }

            // ── Hypervisor & Hardware Telemetry Strip ──
            RowLayout {
            ...
            RowLayout {
                ...
                DialogButton {
                    text: "Configured Machines (" + (root.vmData.vms ? root.vmData.vms.length : 0) + ")"
                    primary: root.activeTab === "vms"
                    onClicked: root.activeTab = "vms"
                }

                DialogButton {
                    text: "Deploy Operating System"
                    primary: root.activeTab === "catalog"
                    onClicked: root.activeTab = "catalog"
                }

                DialogButton {
                    text: "Custom ISO Image"
                    primary: root.activeTab === "custom"
                    onClicked: root.activeTab = "custom"
                }
            ...
            // ═════════════════════════════════════════════════════════════════
            // TAB 1: CONFIGURED VIRTUAL MACHINES
            // ═════════════════════════════════════════════════════════════════
            ColumnLayout {
                Layout.fillWidth: true
                spacing: 12
            ...
            // ═════════════════════════════════════════════════════════════════
            // TAB 2: PRECONFIGURED OS CATALOG (QUICKGET)
            // ═════════════════════════════════════════════════════════════════
            ColumnLayout {
                Layout.fillWidth: true
                spacing: 14
    ...
    Rectangle {
        anchors.fill: parent
        color: Theme.withAlpha(Theme.bg, 0.82)
        ...
        Rectangle {
            ...
            ColumnLayout {
                ...
                RowLayout {
                    Layout.fillWidth: true; spacing: 10
                    Text {
                        text: "Provision " + (root.targetPreset ? root.targetPreset.name : "Virtual Machine")
                        color: Theme.text; font.family: Theme.fontFamily; font.pixelSize: Theme.fontSizeHeader; font.bold: true
                        Layout.fillWidth: true
                    }
                    DialogButton {
                        text: "✕"
