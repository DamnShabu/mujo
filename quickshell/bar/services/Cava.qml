...
// CavaWidget (desktop) and the island's cava-mini module both render from it.
// Runs only when something wants it AND audio is actually playing/unmuted.
QtObject {
    id: cava

    // Desktop cava widgets register themselves; presence of a widget IS the
    // intent, so there is no separate enable setting to keep in sync with it.
    property int widgetWants: 0
    function acquire() { cava.widgetWants += 1 }
    function release() { cava.widgetWants = Math.max(0, cava.widgetWants - 1) }
    ...
    property Process _proc: Process {
        ...
        command: ["sh", "-c", "f=$(mktemp) || exit 1; trap 'rm -f \"$f\"' EXIT INT TERM; printf '%s' \"$1\" > \"$f\"; cava -p \"$f\"", "sh",
