...
# The six biggest subcommands live in sourced libraries so this file stays a
# dispatcher. Set by the Nix wrapper; the fallback is the working tree, so
# `bash quickshell/mujo.sh …` works without installing anything.
MUJO_LIB="${MUJO_LIB:-$(cd "$(dirname "$(readlink -f "${BASH_SOURCE[0]}")")/lib" && pwd)}"
