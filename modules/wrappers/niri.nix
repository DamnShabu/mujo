  ...
  flake.wrappers.niri = {
    ...
    in {
      settings = {
        ...
        layout = {
          # gaps + struts is how far a window sits from the edge of the usable
          # area. quickshell insets the desktop surface by the same total
          # (Theme.desktopInset, default 20) so icons and widgets stay inside the
          # rectangle an open window covers — change these and change that too.
          gaps = 10;
        ...
        workspaces = let
          enabledOutputs = lib.filterAttrs (_name: o: o.enabled or true) (ns.outputs or {});
          outputNames = builtins.attrNames enabledOutputs;
          mkWorkspace = output: idx: {
            name = "${output}-${toString idx}";
            value = {
              open-on-output = output;
              layout.gaps = 5;
            };
          };
          fallbackWorkspaces = lib.listToAttrs (
            map (i: {
              name = "w${toString i}";
              value = {layout.gaps = 5;};
            }) (lib.range 1 4)
          );
          outputWorkspaces = lib.listToAttrs (
            lib.concatMap (
              out: map (idx: mkWorkspace out idx) (lib.range 1 4)
            )
            outputNames
          );
        in
          if outputNames == []
          then fallbackWorkspaces
          else outputWorkspaces;
